...
{
    /// <summary>
    /// Creates new <see cref="IAction"/>s for the <see cref="BrushTool{T}"/>.
    /// </summary>
    public class BrushToolActionFactory<T> : IToolActionFactory<T> where T : IComparable
    {
        ...
        {
            return new UseToolAction<T>(tool as BrushTool<T>, grid, position, value, lastValue, graphicValue, lastGraphicValue, layer);
        }
    }
}