...
{
    /// <summary>
    /// Updates layers of the <see cref="InteractableEditorGridV2"/>.
    /// </summary>
        ...
        public Sprite Build<T>(ObjectGrid<string> IDGrid, AssetList<T> assetList) where T : AssetBase
        {
            ...
            for (int y = 0; y < size.y; y++)
            {
                for (int x = 0; x < size.x; x++)
                {
                    ...
                    if (id == EditorDefaults.EmptyAssetID) continue;

                    if (lastAsset != null && lastAsset.ID == id)
                    {
                        UpdateValue(sprite, new Vector2Int(x, y), lastAsset.Icon);
                    }
                    
                    try
                    {
                        lastAsset = assetList.GetByID(id);
                        UpdateValue(sprite, new Vector2Int(x, y), lastAsset.Icon);
                    }
                    catch (InvalidOperationException)
                    {
                        UpdateValue(sprite, new Vector2Int(x, y), EditorDefaults.MissingSprite);
                    }
                }
            }
