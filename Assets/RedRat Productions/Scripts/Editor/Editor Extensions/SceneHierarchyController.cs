...
namespace RedRats.Editor.UnityEditorExtensions
{
    ...
    /// </summary>
    public static class SceneHierarchyController
    {
        /// <summary>
        /// Selects a <see cref="GameObject"/> in the Hierarchy window.
        /// </summary>
        /// <param name="gameObject">The <see cref="GameObject"/> to select.</param>
        public static void Select(GameObject gameObject) => Selection.activeGameObject = gameObject;
        ...
        {
            object sceneHierarchy = GetSceneHierarchy();
            MethodInfo methodInfo = sceneHierarchy.GetType().GetMethod("ExpandTreeViewItem", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        {
            object sceneHierarchy = GetSceneHierarchy();
            MethodInfo methodInfo = sceneHierarchy.GetType().GetMethod("SetExpandedRecursive", BindingFlags.Public | BindingFlags.Instance);
            methodInfo?.Invoke(sceneHierarchy, new object[] {go.GetInstanceID(), expand});
        }
